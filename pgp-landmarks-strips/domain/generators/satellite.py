...
def generate_domain():
   str_out = ""
   str_out += "(define (domains SATELLITE)\n"
...
def main():
   ...
   f_domain=open( out_folder + "domains.pddl", "w" )
   ...
   for i in range(from_nth,to_nth+1):
      ...
      reader.parse_domain(out_folder +"domains.pddl")

      problem = reader.parse_instance(out_folder + str( i+1-from_nth ) + ".pddl")
      PDDLtranslator.translate_domain(problem,extra_pointers,out_folder +"domains.txt")
