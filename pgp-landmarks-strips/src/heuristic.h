...
public:
	...
	// This assumes state variables are in the same domains and goals as equalities
	int compute( Program *p, GeneralizedPlanningProblem *gpp ) override{
...
class Hplusvisit : public Heuristic{
public:
    Hplusvisit() : Heuristic( "htest" ){}

    virtual ~Hplusvisit(){}

    virtual int compute( Program *p, GeneralizedPlanningProblem *gpp = NULL ){
//        StateDescriptor *sd = gpp->getDomain()->getStateDescriptor();
//        vector<ProgramState*> vps = p->run_heuritsic( gpp );
        int res=0;
        int problem_size=1000;
        int W = 1; // default value
        int test_number=W*p->getNumOftest();
        res += problem_size-test_number;
        return res;

    }
};

class Hplusrelated : public Heuristic{
public:
    Hplusrelated() : Heuristic( "hunrelated" ){}

    virtual ~Hplusrelated(){}

    virtual int compute( Program *p, GeneralizedPlanningProblem *gpp = NULL ){
//        StateDescriptor *sd = gpp->getDomain()->getStateDescriptor();
//        vector<ProgramState*> vps = p->run_heuritsic( gpp );
        int res=0;// default value
        res= p->getNumOfrelatedActions();
        return res;
...
class HpluscmpLocation : public Heuristic{
public:
    HpluscmpLocation() : Heuristic( "hcmplocation" ){}

    virtual ~HpluscmpLocation(){}

    virtual int compute( Program *p, GeneralizedPlanningProblem *gpp = NULL ){
        int res=1000-p->getNumOfcmpLocation();
        return res;

    }
};

class HpluscmpPointer : public Heuristic{
public:
    HpluscmpPointer() : Heuristic( "hcmppointer" ){}

    virtual ~HpluscmpPointer(){}

    virtual int compute( Program *p, GeneralizedPlanningProblem *gpp = NULL ){
        int res=1000-p->getNumOfcmpPointer();
        return res;
...
class Hplusnovelty : public Heuristic {
public:
    Hplusnovelty() : Heuristic("hnovelty") {}

    virtual ~Hplusnovelty() {}

    virtual int compute(Program *p, GeneralizedPlanningProblem *gpp = NULL) {
        int res=0;

        for (auto &actions : p->getMap()) {
            if (actions.second > 0) {
                if (actions.first.find("test")!=std::string::npos or actions.first.find("cmp")!=std::string::npos)
                    continue;
                res += actions.second - 1;
            }
        }
    ...
    virtual int compute( Program *p, GeneralizedPlanningProblem *gpp = NULL ){
        ...
        for( int i = 0; i < int( instructions.size() ); i++ ){
            if( instructions[ i ] != NULL){
                ...
                if (a and a->getName().find("test")==std::string::npos and a->getName().find("cmp")==std::string::npos){
                    res++;
                }
            }
        }
        return res;
//        return p->getSet().size()+p->getPlan().size()+p->getPointers().size();
    }
