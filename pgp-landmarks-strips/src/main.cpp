...
int main(  int argc, const char* argv[] ){
    if( argc < 6 ){
        ...
        cout << "[INFO] List of heuristics is optional, the domains file must be domains.txt with all instances numbered from 1.txt to [n].txt" << endl;
        return -1;
    }
    ...
    string problem_folder = string( argv[4] );

    string outfile = string(argv[5]);

    vector< string > heuristic_names( argc - 6 );
    string heuristics;
    bool preprocess_landmarks = false;
    for( int i = 6; i < argc; i++ ) {
        heuristic_names[ i-6 ] = string(argv[i]);
        heuristics += "_" + heuristic_names[i-6];
        if( heuristic_names[ i-6 ] == "landmarks" or heuristic_names[i-6] == "normlandmarks")
            preprocess_landmarks = true;
    }
    if( argc == 6 ){ // default heuristic
        heuristic_names.emplace_back("landmarks" );
        preprocess_landmarks = true;
        heuristics = "_landmarks";
    }
    if( heuristics=="" ){ // default heuristic
        preprocess_landmarks = false;
    }
...
//        string outfile = "experiments/" + dom->getName() + "_" + search_mode + "_" + to_string(program_lines) +"_"+to_string(width_line) + heuristics;
//    string outfile =  dom->getName() + "_" + search_mode + "_" + to_string(program_lines) + heuristics;
//    if( problem_pos != string::npos ){
//        outfile = problem_folder.substr( problem_pos + 1, problem_folder.length() - 2 - problem_pos )
...
//                + "_" + search_mode + "_" + to_string( program_lines ) + heuristics;
//        outfile = "experiments/" + problem_folder.substr( problem_pos + 1, problem_folder.length() - 2 - problem_pos )
//                  + "_" + search_mode + "_" + to_string( program_lines ) +"_"+to_string(width_line) + heuristics;
//
//    }
    ...
    if (CHECK_RELAXEDACTION) {
        engine->checkActionsBackward();
        if (gd->getRelaxedActions().size()==0){
            return 1;
        }
    }
    ...
    for( const auto& h_name : heuristic_names ){
        ...
        else if( h_name == "hcount" ) engine->addHeuristic( new Hpluscount() );
        else if( h_name == "htest" ) engine->addHeuristic( new Hplusvisit() );
        else if( h_name == "hnovelty" ) engine->addHeuristic( new Hplusnovelty() );
        else if( h_name == "hline" ) engine->addHeuristic( new Hplusline() );
        else if( h_name == "hlocation" ) engine->addHeuristic( new HpluscmpLocation() );
        else if( h_name == "hpointer" ) engine->addHeuristic( new HpluscmpPointer() );
        else if( h_name == "hunrelated" ) engine->addHeuristic( new Hplusrelated() );
